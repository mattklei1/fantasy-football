...
def _build_claude_prompt(facts: dict) -> str:
    ...
    if bad_beat and "player_name" in bad_beat:
        ...
        bad_beat_instructions = (
            ...
            "beat' storyline). This section is ABOUT ONE SPECIFIC TEAM: `awards.bad_beat.team` (team_pk "
            f"{bad_beat_team_pk!r} - the highest scorer among that week's real losing teams; player-level "
    ...
    other_instructions = (
        "Whenever you cite an ALL-PLAY record (`all_play_wins`/`all_play_losses` on `awards.unluckiest_loss` "
        "or `awards.luckiest_win`, if you use them at all), use those two numbers exactly as given - never "
        "derive the loss count from the league's team count. All-play is a record against every OTHER team "
        "that week (n-1 opponents, a team never plays itself), not out of n.\n\n"
