...
week's ESPN projection with ESPN's season-long positional rank, weighted
across starters vs. bench with a season-position-aware split. See
...
st.caption(
    ...
    "Blends this week's ESPN projection with ESPN's season-long positional rank."
)
...
with st.expander("Methodology"):
    st.markdown(
        """
Each rostered player gets a blended **player value** (0-1):
- **This week's ESPN projection**, converted to a percentile *within their position* across
  every rostered player league-wide (so a QB's projection is only compared to other QBs)
- **ESPN's season-long positional rank**, converted to a bounded 0-1 score via
  `1 / (1 + (rank-1)/12)` (rank 1 → 1.0, rank 13 → 0.5, rank 25 → 0.33, ...)
- Weighted 20:15 (projection : rank) - carried over proportionally from a broader 4-source
  design (FantasyPros and Yahoo rest-of-season rankings were considered but aren't wired up:
  FantasyPros' Terms of Use prohibit automated scraping without a licensed API, and Yahoo has
  no generic rankings endpoint outside of league-specific OAuth access)
